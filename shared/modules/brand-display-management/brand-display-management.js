(function () {
  ...
  function newNotice() {
    return { id: '', title: '', type: '排课通知', author: '刘老师', publishTime: '', status: '草稿', pin: false, content: '' };
  }
  ...
  function renderShell() {
    ...
    main.innerHTML = '<div id="bdm-app" v-cloak>' + getTemplate() + '</div>';
        ...
        setup: function () {
          ...
          var stats = Vue.computed(function () {
            ...
            return {
              ...
              notices: notices.value.filter(function (item) { return item.status === '已发布'; }).length
            };
          });
  ...
  function getTemplate() {
    return [
      ...
      '    <div><p class="bdm-kicker">空间与排课管理 / 信息发布</p><h1 class="bdm-title">班牌和信息大屏管理</h1><p class="bdm-subtitle">统一维护教室门口班牌、楼层信息大屏和通知公告，列表内展示基础信息与画面缩略图。</p></div>',
      '    <div class="bdm-actions"><a-button type="primary" @click="openDevice(\'create\', activeTab)"><template #icon><icon-plus /></template>{{ activeTab === \'sign\' ? \'新建班牌\' : \'新建信息大屏\' }}</a-button><a-button @click="openNotice(\'create\')"><template #icon><icon-notification /></template>新建通知</a-button></div>',
      '  </header>',
      '  <div class="bdm-stat-grid"><div class="bdm-stat"><span>班牌总数</span><strong>{{ stats.signs }}</strong></div><div class="bdm-stat"><span>信息大屏</span><strong>{{ stats.screens }}</strong></div><div class="bdm-stat"><span>启用设备</span><strong>{{ stats.enabled }}</strong></div><div class="bdm-stat"><span>已发布通知</span><strong>{{ stats.notices }}</strong></div></div>',
      '  <section class="bdm-card">',
      '    <div class="bdm-toolbar"><a-tabs :active-key="activeTab" @change="tabChanged"><a-tab-pane key="sign" title="班牌管理"></a-tab-pane><a-tab-pane key="screen" title="信息大屏"></a-tab-pane><a-tab-pane key="notice" title="通知公告"></a-tab-pane></a-tabs><div v-if="activeTab !== \'notice\'" class="bdm-filters"><a-input-search v-model="keyword" allow-clear placeholder="请输入名称、序列号或位置" style="width:260px" /><a-select v-model="statusFilter" placeholder="启用状态" allow-clear style="width:128px"><a-option value="true">启用</a-option><a-option value="false">停用</a-option></a-select><a-button @click="resetFilters"><template #icon><icon-refresh /></template>重置</a-button></div></div>',
      '    <div v-if="activeTab !== \'notice\'" class="bdm-table-wrap"><a-table :data="filteredDevices" :pagination="{ pageSize: 6, showTotal: true }" row-key="id" :bordered="false"><template #columns><a-table-column title="画面" :width="150"><template #cell="{ record }"><div class="bdm-thumb" :class="record.imageType"></div></template></a-table-column><a-table-column title="基本信息" :width="230"><template #cell="{ record }"><div class="bdm-name-cell"><strong>{{ record.name }}</strong><span>{{ record.serial }}</span></div></template></a-table-column><a-table-column title="绑定位置" data-index="location"></a-table-column><a-table-column title="类型" :width="130"><template #cell="{ record }"><span class="bdm-tag" :class="{ pink: record.type.indexOf(\'大厅\') > -1, green: record.type.indexOf(\'班牌\') > -1 }">{{ record.type }}</span></template></a-table-column><a-table-column title="模板" data-index="template" :width="150"></a-table-column><a-table-column title="状态" :width="100"><template #cell="{ record }"><span class="bdm-status"><i class="bdm-dot" :class="{ off: !record.enabled }"></i>{{ record.enabled ? \'启用\' : \'停用\' }}</span></template></a-table-column><a-table-column title="操作" :width="260" align="center"><template #cell="{ record }"><span class="bdm-row-actions"><a-button type="text" size="small" @click="openDevice(\'view\', activeTab, record)">详情</a-button><a-button type="text" size="small" @click="openDevice(\'edit\', activeTab, record)">编辑</a-button><a-button type="text" size="small" @click="toggleDevice(record)">{{ record.enabled ? \'停用\' : \'启用\' }}</a-button><a-popconfirm content="确认删除这条记录？" @ok="removeDevice(activeTab, record.id)"><a-button type="text" size="small" status="danger">删除</a-button></a-popconfirm></span></template></a-table-column></template></a-table></div>',
      '    <div v-else class="bdm-table-wrap"><a-table :data="notices" :pagination="{ pageSize: 6, showTotal: true }" row-key="id" :bordered="false"><template #columns><a-table-column title="标题" data-index="title"></a-table-column><a-table-column title="类型" data-index="type" :width="120"></a-table-column><a-table-column title="发布人" data-index="author" :width="100"></a-table-column><a-table-column title="发布时间" data-index="publishTime" :width="160"></a-table-column><a-table-column title="状态" :width="110"><template #cell="{ record }"><a-tag :color="record.status === \'已发布\' ? \'green\' : \'orange\'">{{ record.status }}</a-tag></template></a-table-column><a-table-column title="操作" :width="250" align="center"><template #cell="{ record }"><span class="bdm-row-actions"><a-button type="text" size="small" @click="openNotice(\'view\', record)">详情</a-button><a-button type="text" size="small" @click="openNotice(\'edit\', record)">编辑</a-button><a-button v-if="record.status !== \'已发布\'" type="text" size="small" status="success" @click="publishNotice(record.id)">发布</a-button><a-popconfirm content="确认删除这条通知？" @ok="removeNotice(record.id)"><a-button type="text" size="small" status="danger">删除</a-button></a-popconfirm></span></template></a-table-column></template></a-table></div>',
      '  </section>',
      ...
      '  <a-drawer v-model:visible="noticeDrawer" :width="620" :title="noticeMode === \'view\' ? \'通知详情\' : (noticeMode === \'edit\' ? \'编辑通知\' : \'新建通知\')" unmount-on-close><div class="bdm-notice-layout"><div v-if="noticeMode === \'view\'" class="bdm-section"><div class="bdm-section-title">{{ noticeForm.title }}</div><dl class="bdm-read-grid"><dt>类型</dt><dd>{{ noticeForm.type }}</dd><dt>发布人</dt><dd>{{ noticeForm.author }}</dd><dt>发布时间</dt><dd>{{ noticeForm.publishTime }}</dd><dt>状态</dt><dd>{{ noticeForm.status }}</dd><dt>置顶</dt><dd>{{ noticeForm.pin ? \'是\' : \'否\' }}</dd></dl></div><a-form v-else :model="noticeForm" layout="vertical"><a-form-item label="公告标题" required><a-input v-model="noticeForm.title" placeholder="请输入公告标题" /></a-form-item><div class="bdm-form-grid"><a-form-item label="公告类型"><a-select v-model="noticeForm.type"><a-option value="排课通知">排课通知</a-option><a-option value="设备维护">设备维护</a-option><a-option value="考核公告">考核公告</a-option><a-option value="其他">其他</a-option></a-select></a-form-item><a-form-item label="发布人"><a-input v-model="noticeForm.author" /></a-form-item></div><a-form-item label="公告内容"><a-textarea v-model="noticeForm.content" :auto-size="{ minRows: 8, maxRows: 12 }" placeholder="请输入通知内容" /></a-form-item><a-form-item><a-checkbox v-model="noticeForm.pin">置顶展示到班牌和信息大屏</a-checkbox></a-form-item></a-form><div class="bdm-notice-detail">{{ noticeForm.content || \'暂无通知内容\' }}</div></div><template #footer><a-button @click="noticeDrawer = false">关闭</a-button><a-button v-if="noticeMode !== \'view\'" @click="saveNotice(false)">保存草稿</a-button><a-button v-if="noticeMode !== \'view\'" type="primary" @click="saveNotice(true)">立即发布</a-button></template></a-drawer>',
      '</section>'
    ].join('');
  }
